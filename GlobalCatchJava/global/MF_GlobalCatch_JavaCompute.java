...
public class MF_GlobalCatch_JavaCompute extends MbJavaComputeNode {
		...
		try {
			...
			try {
				...
				if(globalMap.containsKey(ID)) {
					globalMap.update(message, ID);
					
				}else {
					globalMap.put(message,ID);
				}
			}
			catch (Exception e) {
			...
			MbElement outputroot = outMessage.getRootElement().createElementAsLastChild(MbXMLNSC.PARSER_NAME).createElementAsLastChild(MbXMLNSC.FOLDER,"Reesponse",null);
			outputroot.createElementAsLastChild(MbXMLNSC.FOLDER,"Message","Data loaded Successfully");
